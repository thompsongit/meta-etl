...
DEFAULT_ENV_CANDIDATES = (
    Path(".prod.fb.env"),
    Path("scratch/.prod.fb.env"), # for testing purposes
)
...
class SyncConfig:
    ...
    comments_post_scan_limit: int
    comments_lookback_hours: int
    comments_backfill_days: int
    disable_page_insights: bool
    disable_post_insights: bool
    max_post_insight_requests: int
...
def _parse_utc_datetime(value: str | None) -> datetime | None:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid datetime format for FB_INITIAL_SYNC_START_AT/--initial-sync-start-at: {value}"
        ) from exc
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Facebook Pages -> ClickHouse incremental sync")
    parser.add_argument(
        "--env-file",
        default=None,
        help="Optional env file path. If omitted, auto-loads .prod.fb.env/.prod.env candidates",
    )
    parser.add_argument(
        "--override-env",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "--graph-token",
        dest="graph_token",
        default=os.getenv("FB_GRAPH_TOKEN"),
    )
    parser.add_argument(
        "--graph-base",
        default=os.getenv("FB_GRAPH_BASE", os.getenv("GRAPH_BASE", DEFAULT_GRAPH_BASE)),
    )
    parser.add_argument(
        "--graph-version",
        default=os.getenv("FB_GRAPH_VERSION", os.getenv("GRAPH_VERSION", DEFAULT_GRAPH_VERSION)),
    )

    parser.add_argument(
        "--backfill-days",
        type=int,
        default=int(os.getenv("FB_BACKFILL_DAYS", os.getenv("BACKFILL_DAYS", "90"))),
    )
    parser.add_argument(
        "--initial-sync-start-at",
        default=os.getenv("FB_INITIAL_SYNC_START_AT", os.getenv("INITIAL_SYNC_START_AT")),
        help="UTC ISO datetime for initial bootstrap start, e.g. 2024-01-01T00:00:00Z",
    )
    parser.add_argument(
        "--backfill-chunk-days",
        type=int,
        default=int(os.getenv("FB_BACKFILL_CHUNK_DAYS", os.getenv("BACKFILL_CHUNK_DAYS", "60"))),
    )
    parser.add_argument(
        "--max-windows-per-run",
        type=int,
        default=int(os.getenv("FB_MAX_WINDOWS_PER_RUN", os.getenv("MAX_WINDOWS_PER_RUN", "0"))),
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(
            os.getenv("FB_SYNC_LOOKBACK_HOURS", os.getenv("SYNC_LOOKBACK_HOURS", "72"))
        ),
    )
    parser.add_argument(
        "--posts-page-size",
        type=int,
        default=int(os.getenv("FB_POSTS_PAGE_SIZE", "100")),
    )
    parser.add_argument(
        "--disable-comments",
        action="store_true",
        default=_parse_bool(os.getenv("FB_DISABLE_COMMENTS"), default=False),
    )
    parser.add_argument(
        "--comments-page-size",
        type=int,
        default=int(os.getenv("FB_COMMENTS_PAGE_SIZE", "100")),
    )
    parser.add_argument(
        "--comments-post-scan-limit",
        type=int,
        default=int(os.getenv("FB_COMMENTS_POST_SCAN_LIMIT", "200")),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=int(
            os.getenv(
                "FB_COMMENTS_BACKFILL_DAYS",
                os.getenv("FB_BACKFILL_DAYS", os.getenv("BACKFILL_DAYS", "90")),
            )
        ),
    )
    parser.add_argument(
        "--disable-page-insights",
        action="store_true",
        default=_parse_bool(os.getenv("FB_DISABLE_PAGE_INSIGHTS"), default=False),
    )
    parser.add_argument(
        "--disable-post-insights",
        action="store_true",
        default=_parse_bool(os.getenv("FB_DISABLE_POST_INSIGHTS"), default=False),
    )
    parser.add_argument(
        "--max-post-insight-requests",
        type=int,
        default=int(os.getenv("FB_MAX_POST_INSIGHT_REQUESTS", "200")),
    )
    parser.add_argument(
        "--lock-file",
        default=os.getenv("FB_SYNC_LOCK_FILE", "/tmp/fb_etl_sync.lock"),
    )
    parser.add_argument(
        "--http-timeout-seconds",
        type=int,
        default=int(os.getenv("FB_HTTP_TIMEOUT_SECONDS", os.getenv("HTTP_TIMEOUT_SECONDS", "45"))),
    )
...
def parse_config(argv: Sequence[str] | None = None) -> SyncConfig:
    ...
    return SyncConfig(
        page_id=_require("FB_PAGE_ID/--fb-page-id", args.fb_page_id),
        graph_token=_require("FB_GRAPH_TOKEN/--fb-graph-token", args.graph_token),
        graph_base=args.graph_base,
        graph_version=args.graph_version,
        backfill_days=max(1, args.backfill_days),
        lookback_hours=max(1, args.lookback_hours),
        posts_page_size=max(1, min(args.posts_page_size, 100)),
        disable_comments=bool(args.disable_comments),
        comments_page_size=max(1, min(args.comments_page_size, 100)),
        comments_post_scan_limit=max(1, args.comments_post_scan_limit),
        comments_lookback_hours=max(1, args.comments_lookback_hours),
        comments_backfill_days=max(1, args.comments_backfill_days),
        disable_page_insights=bool(args.disable_page_insights),
        disable_post_insights=bool(args.disable_post_insights),
        max_post_insight_requests=max(1, args.max_post_insight_requests),
